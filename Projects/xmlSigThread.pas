...
type
  TThreadParse = class(TThread)
    private
      ...
      FieldFree:^FieldRecord;          //Record指针
...
implementation
uses dataModuleSig,xmlSigRun;

constructor  TThreadParse.Create();
begin
...
function  TThreadParse.checkFileName(Filename:string):Boolean;        //在数据库中检测文件是否存在
var
  sql:string;
begin
  //
...
procedure TThreadParse.SavefileName(Filename:string;cnt:integer);                 //当成功后，将完成的文件写入数据库中
var
  sql:string;
begin
...
procedure TThreadParse.openDB();                                      //打开数据库
var
  sql:string;
begin
...
//每个文件单独运行
procedure TThreadParse.runSingleFile(filename:string);
var
  starttick:Integer;
  endtick:Integer;
...
begin
     ...
     sigRun.mmoDebug.Lines.Append('！！！！进入'+filename+'处理信息,当前时刻为：'+TimeToStr(Time));
     starttickWhole := GetTickCount;

     //初始化
     recordID := 0;
     sigRun.mmoDebug.Lines.Add('-------------------------------------------');
     //XML文件载入
     ...
     try
        ...
        try
           ...
           tmp := '开始载入文件，当前时间为:' + TimeToStr(Time); sigRun.mmoDebug.Lines.Add(tmp);
           starttick := GetTickCount;

           xmlRun.LoadFromFile(filename);  //从指定文件出载入XML ,自动进行解析时候必须符合XML语法规范

           endtick   := GetTickCount;
           tmp := '恭喜成功载入文件，当前时间为:' + TimeToStr(Time); sigRun.mmoDebug.Lines.Add(tmp);
           tmp := '-->载入文件，用时:'+floattostr((endtick- starttick)/1000.0)+'秒'; sigRun.mmoDebug.Lines.Add(tmp);
        except;
        ...
        //step2: 创建数据存储文件
        storeFileName := 'D:\处理数据\EP\' + extractfilename(filename) + '.tianwei';
        ...
        try
          ...
          //step5: 将每一条记录导入
          tmp := '开始分析记录，当前时间为:' +  TimeToStr(Time); sigRun.mmoDebug.Lines.Add(tmp);
          starttick := GetTickCount;
          ...
          endtick   := GetTickCount;
          tmp := '恭喜成功分析完成所有记录，当前时间为:' + TimeToStr(Time); sigRun.mmoDebug.Lines.Add(tmp);
          tmp := '-->分析文件，用时:'+floattostr((endtick- starttick)/1000.0)+'秒'; sigRun.mmoDebug.Lines.Add(tmp);
     ...
     sigRun.mmoDebug.Lines.Append('(*_*)Success!完成'+filename+'处理信息,结束时刻为：'+TimeToStr(Time)+'处理时间共计'+floattostr(((endtickWhole-starttickWhole)/1000.0)/60)+'分钟');
     sigRun.mmoDebug.Lines.Append('<------------end------------->');
end;

procedure TThreadParse.BatchRun();
var
  I:Integer;
  cnt:Integer;
...
begin
     //开启数据库
     sigRun.mmoDebug.Lines.Add('打开数据库');
     openDB;
     ...
     begin
          ...
          begin
            cnt  := cnt +1;
            //sigRun.mmoDebug.Lines.Add(ParamStr(I));
            sigRun.mmoDebug.Lines.Append('当前正在处理第'+inttostr(I)+'文件');
     ...
     sigRun.mmoDebug.Lines.Append('总共有:'+inttostr(cnt)+'个文件');
end;

procedure TThreadParse.ParseElement(ANode:TXmlNode);       //对每条记录进行解析
var
  cnt:Integer;
...
begin
     ...
     tmpint := 0;
     FillChar(fieldEvery,SizeOf(fieldEvery),0);
     GetMem(FieldFree,SizeOf(fieldEvery));
     //对每一行进行解析，容量大约5M左右
...
procedure TThreadParse.saveDocRecord();                         //存储文档信息储到数据库中
begin
...
function TThreadParse.checkField(InString:string):string;
var
  tmp:string;
begin
     //对写入数据的字符进行转义处理
...
procedure TThreadParse.savetianweiFile();
var
  sql :string;
begin
     //将每一条记录写入txt文本中
